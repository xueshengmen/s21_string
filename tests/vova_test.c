...
START_TEST(strtok_1) {
  ...
  while (nstr_1 != NULL) {
    ...
    nstr_1 = strtok(NULL, del4);
    nstr_2 = s21_strtok(NULL, del4);
  }
}
END_TEST
...
START_TEST(sprintf_test_one_hex_lower) {
  ...
  int b = printf(str2, "%x", (unsigned)INT32_MIN);

  ck_assert_int_eq(a, b);
  ck_assert_str_eq(str1, str2);

  a = s21_sprintf(str1, "%x", 0);
  b = printf(str2, "%x", 0);

  ck_assert_int_eq(a, b);
  ck_assert_str_eq(str1, str2);

  a = s21_sprintf(str1, "%x", INT32_MAX);
  b = printf(str2, "%x", INT32_MAX);
...
START_TEST(sprintf_test_one_hex_upper) {  // original returns trash
  char str1[BUFF_SIZE];
  char str2[BUFF_SIZE];
  int a = s21_sprintf(str1, "%X", (unsigned)INT32_MIN);
  int b = printf(str2, "%X", (unsigned)INT32_MIN);
...
START_TEST(sprintf_test_many_hex_lower) {
  // s21  (bc4efb870fffffb32f8a7eabd) 25
  //  orig (bc4efb870fffffb32f8a7eabd) 25
  char str1[BUFF_SIZE];
  char str2[BUFF_SIZE];
...
Suite *suite_insert(void) {
  ...
  // * - ignore the char
  tcase_add_test(tc, sscanf_chars_flags4);  // Length
  // flags do nothing
  tcase_add_test(tc, sscanf_chars_flags5);  // * flags -
  // ignores char
...
int main(void) {
  int nf;
  SRunner *sr;
  sr = srunner_create(suite_insert());
